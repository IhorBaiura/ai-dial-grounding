...
class SearchRequests(BaseModel):
    search_request_parameters: list[SearchRequest] = Field(
        ...
        description="A list of search requests extracted from the user's query.")  
...
def retrieve_context(user_question: str) -> list[dict[str, Any]]:
    ...
    search_request: SearchRequests = (prompt | llm_client | parser).invoke({})

    if search_request.search_request_parameters:
        requests_dict = {req.search_field.value: req.search_value for req in search_request.search_request_parameters}
        print(f"Extracted search parameters: {requests_dict}")
        users = user_client.search_users(**requests_dict)
        print(f"Retrieved {len(users)} users matching search criteria.")
        return users
    else:
...
def augment_prompt(user_question: str, context: list[dict[str, Any]]) -> str:
    ...
    prompt = USER_PROMPT.format(context=context_str, query=user_question)
